    ...
    return (<div style={{ padding: "var(--space-6)", maxWidth: 1400 }}>
        ...
        <Card padding="none">
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "var(--space-3) var(--space-4)", background: "var(--color-bg-secondary)", borderBottom: "1px solid var(--color-border)" }}>
                <Input type="text" placeholder="Filter jobs..." className="mono" style={{ width: 200, fontSize: "13px" }} />
                <Button variant="ghost" size="sm" icon={<Settings2 size={16} />}>View Options</Button>
            </div>
                ...
                <tbody>
                    {jobs.map(j => (
                        <tr key={j.id} style={{ borderBottom: "1px solid var(--color-border)" }}>
                            ...
                            <td style={{ padding: "14px 16px" }}><Badge variant="secondary">{j.schedule}</Badge></td>
                            ...
                            <td style={{ padding: "14px 16px" }}><Badge variant={j.status === "success" ? "success" : j.status === "warn" ? "warning" : "error"}>{j.status}</Badge></td>
                            <td style={{ padding: "14px 16px", textAlign: "right" }}>
                                <Button variant="outline" size="sm" icon={<Play size={14} />}>Run Now</Button>
                            </td>
                        </tr>
                    ))}
                </tbody>
